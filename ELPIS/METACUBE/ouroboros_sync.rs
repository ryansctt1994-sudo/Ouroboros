...
// FFI-compatible C API for Python integration
#[no_mangle]
pub extern "C" fn sync_engine_new(num_agents: usize) -> *mut SyncEngine {
    Box::into_raw(Box::new(SyncEngine::new(num_agents)))
}

#[no_mangle]
pub extern "C" fn sync_engine_free(engine: *mut SyncEngine) {
    if !engine.is_null() {
        unsafe {
...
#[no_mangle]
pub extern "C" fn sync_engine_synchronize(engine: *mut SyncEngine) {
    if !engine.is_null() {
        unsafe {
            (*engine).synchronize_step();
        }
